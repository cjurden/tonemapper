...
elseif (lightlow <= 0) || (lightlow > 1)
    errordlg('Invalid input. Lightness must be greater than or equal to 0 and less than or equal to 1.','Input Error','modal')
    uicontrol(handles.lightness1)
...
elseif (sat <= 0)
    ...
    uicontrol(handles.saturation)
else %%%If no input errors, Display tonemap.
...
if isnan(sat)
  ...
  return
elseif (sat<=0)
    errordlg('Invalid input. Input has to be greater than or equal to 0','Input Error','modal')
    uicontrol(hObject)
    return
elseif(sat>4 || sat<2)
